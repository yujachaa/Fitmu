          ...
          <div class="popular d-flex align-items-start">
            ...
            <div v-for="product in productScrapList" :key="product.productId" v-else>
              <div class="popular-pic">
                <img class="pic" :src="`src/assets/image/product/${getImage(product.productId)}`" alt="이미지"
...
const getImage = function(id){
  for(let i=0; i<productAllImages.value.length; i++){
    if(productAllImages.value[i].productId == id){
      return productAllImages.value[i].fileName;
    }
  }
}
