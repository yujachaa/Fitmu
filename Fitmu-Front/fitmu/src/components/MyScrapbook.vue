                ...
                <div class="section">
                    <div class="section-title">
                        <div class="small-title">
                            <h4>My Scrap Story</h4>
                        </div>
                        <div>
                            <a @click = "goScrapStory">더보기</a>
                        </div>
                    </div>


                    <div class="popular d-flex align-items-start">
                        <div class="no-story" v-if="noScrapStory">
                            <p>아직 작성한 게시글이 없어요.😅</p>
                        </div>
                        <!-- v-for 넣기 -->
                        <div v-for="story in storyScrapList" :key="story" v-else>
        ...
        <div class="mystory">
            <div>
                <div class="section">
                    <div class="section-title">
                        <div class="small-title">
                            <h4>My Scrap Product</h4>
                        </div>
                        <div>
                            <a @click = "goScrapProduct">더보기</a>
                        </div>
                    </div>


                    <div class="popular d-flex align-items-start">
                        <div class="no-story" v-if="noScrapProduct">
                            <p>아직 작성한 게시글이 없어요.😅</p>
                        </div>
                        <!-- v-for 넣기 -->
                        <div v-for="product in productScrapList" :key="story" v-else>
                            <div class="popular-pic">
                                <img class="pic" :src="`src/assets/image/product/${getImage(product.productId)}`"
                                    alt="이미지" @click="goProduct(product.productId)">
                            </div>
                        </div>
                    </div>
                </div>
...
const productScrapList = computed(() => {
    return userStore.productScrapList.slice(0, 6)
})

const storyScrapList = computed(() => {
    return userStore.storyScrapList.slice(0, 6)
})
...
const goScrapProduct = function(){
    router.push({ name : 'zzim'})
}
const goScrapStory = function(){
...
const noScrapStory = computed(() => {
    return storyScrapList.value.length == 0 ? true : false
})
const noScrapProduct = computed(() => {
    return productScrapList.value.length == 0 ? true : false
})
